"""
Lesson 9 Assignment: Accuracy Measures
Add the following to Lesson 8 Assignment:
1. Confusion Matrix from your predicted values (specify and justify the probability threshold).
2. Precision, Recall, and F1 measures based on your Confusion Matrix.
3. Calculate the ROC curve and it's AUC using sklearn. Present the ROC curve. Present the AUC in the ROC's plot.
...
##########################################################################
# 3. Calculate the ROC curve and it's AUC using sklearn. Present the ROC curve. 
...
# please see comments in code blocks

##########################################################################
# 5. Add a summary comment block that discusses your classification and its accuracy.
##########################################################################

# Summary: A logistic regression classifier was used for a binary classification problem:
# to predict whether income exceeds $50K/yr using the Adult dataset.
# The column class_>50K was used as the expert label (corresponding to "class" column in the original dataset).
# Preliminary analysis returned an overall accuracy of 0.85 using the default probability threshold (classification threshold) of 0.5.
# For imbalanced classes, precision and recall could provide more insight than accuracy. 
# Part of choosing the classification threshold depends on the cost of misclassifications.
# The ROC curve was used to help pick a probability threshold. The general guideline was used:
# opting for the threshold that is based on the point in an ROC curve that is closest to the top-left corner.
# Based on the ROC curve and distance to the top-left corner, a threshold of 0.3 was used.
# From the resultant confusion matrix: precision=0.57, recall=0.79, F1=0.66 for outcome/class 1. Overall accuracy=0.81.
